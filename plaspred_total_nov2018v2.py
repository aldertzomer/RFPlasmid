...
df1a['source'] = df1a.genome.astype(str).str.cat(df1a.contig.astype(str), sep='_')
df1a = df1a.set_index('source')

#import contigIDnumbers
df1b = pd.read_csv('contigID.out', header=None, sep=',', names = ['genome', 'contig', 'contigID'])
df1b['source'] = df1b.genome.astype(str).str.cat(df1b.contig.astype(str), sep='_')
df1b = df1b.set_index('source')
df1 = pd.merge(df1a, df1b, how='outer')
...
if args.jelly:
	...
	df4[['genome','contig']] = df4['source'].str.split('_',expand=True)
if not (args.jelly):
